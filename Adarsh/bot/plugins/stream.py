...
async def login_handler(c: Client, m: Message):
    try:
        try:
            ag = await m.reply_text("this feature has been deleted \n\n (banned..unwanted message = ban + report)")
...
async def private_receive_handler(c: Client, m: Message):
    if MY_PASS:
        ...
        if check_pass== None:
            await m.reply_text("this feature has been banned sorry \n\n (deleted..unwanted message = ban + report)")
            return
...
async def channel_receive_handler(bot, broadcast):
    if MY_PASS:
        ...
        if check_pass == None:
            await broadcast.reply_text("from @jackeyBots")
            return
        if check_pass != MY_PASS:
